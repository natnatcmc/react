...
function ConversorDeMoeda() {
    ...
    const converterMoeda = () => {
        fetch('https://api.exchangerateapi.io/latest?base=${deMoeda}&symbols=${paraMoeda}')
        .then(response => response.json())
        .then (data => {
    ...
    return (
        ...
        <div>
            ...
            <label>Para:</label>
            <select value={paraMoeda} onChange={(e) =>
                setParaMoeda(e.target.value)}>
                    {moedas.map(moeda=> (
                        <option key={moeda}
                        value={moeda}>{}moeda</option>
                    ))}
